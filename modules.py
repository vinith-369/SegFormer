...
class DWConv(keras.layers.Layer):
    ...
    def call(self, x, H, W):    
        get_shape_1 = ops.shape(x)      #(Batch, NumPatches, Channels) => (B, N, C)
        x = ops.reshape(x, (get_shape_1[0], H, W, get_shape_1[-1]))     #(B, N, C) -> (B, H, W, C)
        x = self.dwconv(x)
        get_shape_2 = ops.shape(x)
        x = ops.reshape(
            x, (get_shape_2[0], get_shape_2[1] * get_shape_2[2], get_shape_2[3])    #(B, H, W, C) -> (B, N, C)
        )
        return x        #(B, N, C)
...
class Mlp(keras.layers.Layer):
    ...
    def call(self, x, H, W):
        x = self.fc1(x)     # first Dense layer - expandes
        x = self.dwconv(x, H=H, W=W)    #DWConv
        x = self.act(x)     # GELU
        x = self.drop(x)    #dropout
        x = self.fc2(x)     # second Dense layer - contract
        x = self.drop(x)    #dropout
        return x
...
class Block(keras.layers.Layer):
    ...
    def call(self, x, H, W):
        # LayerNormalization and Attention layer
        ...
        # LayerNormalization and MLP (Mix-FFN) layer
...
class OverlapPatchEmbed(keras.layers.Layer):
    ...
    def call(self, x):  #(B, H_in, W_in, C_in)
        x = self.conv(self.pad(x))      #(B, H_out, W_out, C_out (filters))
        ...
        x = ops.reshape(x, (-1, H * W, C))      #(B, H, W, C) -> (B, H*W, C)
        x = self.norm(x)
        return x, H, W
...
class MixVisionTransformer(keras.layers.Layer):
    def __init__(
        self,
        img_size=224,
        embed_dims=[64, 128, 256, 512],      # channel dimensions for f1,f2,f3,f4 
        num_heads=[1, 2, 4, 8],         # Num heads for f1,f2,f3,f4
        mlp_ratios=[4, 4, 4, 4],
        qkv_bias=False,
        drop_rate=0.0,
        attn_drop_rate=0.0,
